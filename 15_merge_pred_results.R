...
colcol<-read.table("~/mnt/agroscope_os/2/2/6/1/1/4/6021/GenSel_Costa_collab/R_output/predictions/COLLtoCOLL/accuracy_5fold_rrBLUP_newPC12_values.txt", h=T)
...
CC$Model<-"NO_CLUST"
colcolcluster<-read.table("~/mnt/agroscope_os/2/2/6/1/1/4/6021/GenSel_Costa_collab/R_output/predictions/COLLtoCOLL/rrBLUPs_5fold_5clusters_fix_newPC12_values.txt", h=T)
...
bmp(file=paste0(odir, "/predictions/all_scenarios.bmp"), height = 600, width = 1200, res=100)
# pdf(file=paste0(odir, "/predictions/all_scenarios.pdf"), height = 6, width = 12,)
ggplot(all,aes( y=accuracy, x=trait, fill=scenario))+
  geom_bar(stat="identity",position=position_dodge()) +
  facet_wrap(~FAM)+
   labs(x="Trait", title="Predictions rrBLUP", y="Predictive ability")+
  theme_minimal()+
  theme(axis.text.x=element_text(angle = 45,  hjust = 1),
        strip.text.x = element_text(size = 15, face = "bold"),
        legend.title = element_blank()) +      
  scale_y_continuous(limits = c(-0.5, 1)) +
  geom_errorbar(aes(ymin=accuracy-sd, ymax=accuracy+sd), size=0.1, width=0.4,
                position=position_dodge(.9)) 
dev.off()
...
bmp(file=paste0(odir, "/predictions/all_scenarios_clusters.bmp"), height = 600, width = 1200, res=100)
# pdf(file=paste0(odir, "/predictions/all_scenarios_clusters.pdf"), height = 6, width = 12)

ggplot(all,aes( y=accuracy, x=trait, fill=scenario))+
  geom_bar(stat="identity",position=position_dodge()) +
  facet_wrap(~FAM)+
  labs(x="Trait", title="Predictions rrBLUP with 5 clusters", y="Predictive ability")+
  theme_minimal()+
  theme(axis.text.x=element_text(angle = 45,  hjust = 1),
        strip.text.x = element_text(size = 15, face = "bold"),
        legend.title = element_blank()) +      
