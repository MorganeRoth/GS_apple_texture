...
assi<- rbind(dapc1$posterior, pred.sup$posterior)
rownames(assi)<-c(rownames(dapc1$posterior), rownames(pred.sup$posterior))
assi<-assi[ids,]
WhichCOL<-ids[-c(lapply(families, function(x) grep(x, ids) ) %>% unlist)]


################################################################################
### STEP 1 find the optimal training pop with a given number of individuals ####
################################################################################
...
repeatgenalg<-function(numrepsouter,numrepsinner, ntrain){
  ...
  for (i in 1:numrepsouter){
    ...
    StartingPopulation<-lapply(1:numrepsinner, function(x){
      GenAlgForSubsetSelection(P=assi,Candidates=Candidates, 
  ...
  ListTrain<-GenAlgForSubsetSelection(P=assi,Candidates=Candidates, 
...
repeatgenalg0<-function( ntrain, criteria){
  ListTrain<-GenAlgForSubsetSelection(P=assi,Candidates=Candidates, 
                                      Test=Test,ntoselect=ntrain, 
                                      errorstat=criteria, mc.cores=5)
  return(ListTrain)
}
...
pred_fam_trait0 <-function(FAM, trait, criteria) {
  ...
  assiCOLFAM<-assi[WhichCOLFAM, ]
  Test<-rownames(assiCOLFAM)[grep(FAM, rownames(assiCOLFAM))  %>% unlist]
  Candidates<- setdiff(rownames(assiCOLFAM), Test)
  phenos_sub<-phenos[WhichCOLFAM,]
  phenos_sub$id<-rownames(phenos_sub)
  to_test<- seq(10,(length(WhichCOL)),10) ## TRS sizes to test increment of 10
  ListTrain<-lapply(to_test, function(ntrain, criteria){ 
    print(ntrain)
    repeatgenalg0(ntrain)
  } )
  ##predictions by optimized sample
...
for (i in traits) {
  for (j in families) {
    ...
    res_cor[[i]][[j]]<-pred_fam_trait0(j, i, "PVMEAN")
  }
}
