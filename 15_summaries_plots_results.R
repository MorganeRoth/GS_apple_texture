...
colcol<-read.table(paste0(odir, "/predictions/COLLtoCOLL/accuracy_5fold_rrBLUP.txt"), h=T)
...
colcolcluster<-read.table(paste0(odir,"/predictions/COLLtoCOLL/rrBLUPs_5fold_6clusters_fix.txt"), h=T)
...
scenario2<-readRDS(paste0(odir, "/predictions/COLLtoFAMs_prop/all_traits_pred_progenies_0.3_prop100_reps.rds"))
...
scenario3<-read.table(paste0(odir, "/predictions/COLLtoFAMs_add_relFAM/rrBLUPs_aggregated.txt"), h=T)
...
## correct error in some trait names
...
## plot barplot - acknowledge source/help: http://www.sthda.com/french/wiki/ggplot2-barres-d-erreur-guide-de-demarrage-rapide-logiciel-r-et-visualisation-de-donnees
## choose either bmp or pdf format
...
## keep only one trait and compare parameters
...
cat("All results successfully plotted! \n")
